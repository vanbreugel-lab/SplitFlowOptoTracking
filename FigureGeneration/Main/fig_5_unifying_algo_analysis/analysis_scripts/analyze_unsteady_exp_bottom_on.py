...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run unifying algorithm analysis on unsteady experiment bottom-on data.')
    ...
    bottom_on_df = pd.read_hdf(str(config.RAW_DATA_DIR / 'flies_splitflow_bottomon_c1xwt_preprocessed_optotrigger_trimmed.hdf'))
    ...
    flash_low_bottom_on = df_flash_bottom_on.groupby("obj_id_unique_event").filter(lambda d: d.z.max()<0.26)
    flash_low_bottom_on = flash_low_bottom_on[~flash_low_bottom_on.xvel.isna()]

    flash_up_bottom_on  = df_flash_bottom_on.groupby("obj_id_unique_event").filter(lambda d: d.z.min()>0.26)
    flash_up_bottom_on = flash_up_bottom_on[~flash_up_bottom_on.xvel.isna()]

    #

    sham_low_bottom_on = df_sham_bottom_on.groupby("obj_id_unique_event").filter(lambda d: d.z.max()<0.26)
    sham_low_bottom_on = sham_low_bottom_on[~sham_low_bottom_on.xvel.isna()]

    sham_up_bottom_on  = df_sham_bottom_on.groupby("obj_id_unique_event").filter(lambda d: d.z.min()>0.26)
    ...
    # Run analysis on FLASH data LOW FLIES (steady)
    braid_df = flash_low_bottom_on
    ...
    unifying_algo_data.to_parquet(str(config.PREPROCESSED_DATA_DIR / f'steady_bottom_on_data_flash_{translation_flag}.parquet'))


    # Run analysis on SHAM data LOW FLIES (steady)
    braid_df = sham_low_bottom_on
    unifying_algo_data = uaa.run_miop_affine_fit_on_trajectories(braid_df,    
                                                                 n_trajecs,  
                                                                 obj_id_key=obj_id_key,
                                                                 abs_min_ix=abs_min_ix,  
                                                                 ...
                                                                 use_cvx_affine=use_cvx_affine,      
                                                                 include_translation=include_translation)
    unifying_algo_data.to_parquet(str(config.PREPROCESSED_DATA_DIR / f'steady_bottom_on_data_sham_{translation_flag}.parquet'))
    ...
    # all_unifying_algo_data.to_parquet('unsteady_bottom_on_data_flash.parquet')

    # braid_df = sham_up_bottom_on
    # all_unifying_algo_data = run_miop_affine_fit_on_trajectories(braid_df, n_trajecs)
    # all_unifying_algo_data.to_parquet('unsteady_bottom_on_data_sham.parquet')
